  ...
  return (
    <AbsoluteFill style={{ pointerEvents: "none" }}>
      {new Array(lineCount).fill(true).map((_, index) => {
        ...
        const lineWidth = 18 + ((index * 37) % 56);
        const spacing = Math.max(20, Math.floor(height / Math.max(lineCount + 6, 1)));
        const top = 140 + index * spacing;
        const left = index % 2 === 0 ? 0 : width - lineWidth;
