...
$SSH_USER = "ramej"
$SSH_KEY = "$PSScriptRoot\id_rsa_ramej"
...
$zipFile = "$TEMP_DIR\vf_deploy.zip"
...
Write-Host "✅ ZIP creado: $([math]::Round($size,2)) MB" -ForegroundColor Green

# Subir al servidor
Write-Host "⬆️  Subiendo al servidor..." -ForegroundColor Yellow
scp -i $SSH_KEY -o StrictHostKeyChecking=no $zipFile "${SSH_USER}@${SSH_HOST}:/tmp/vf_deploy.zip"
...
unzip -o /tmp/vf_deploy.zip -d $REMOTE_DIR
rm /tmp/vf_deploy.zip
