...
def _normalize_timeline_props(
    ...
    # Preserve vertical/landscape hint so calculateCinematicMetadata picks the
    # right resolution (1080x1920 for shorts vs 1920x1080 for landscape).
    fmt = str(props.get("format", "") or "").strip()
    if fmt in {"9:16", "16:9"}:
        props["format"] = fmt
    else:
        props.pop("format", None)

    resolution = props.get("resolution") if isinstance(props.get("resolution"), dict) else None
    if resolution and resolution.get("width") and resolution.get("height"):
        try:
            props["resolution"] = {
    ...
    # Sanitize dynamic overlays (from tools.graphics.dynamic_overlays via the
    # FullEditingEngine schema bridge). Drop items with invalid timing so
    # Remotion never receives degenerate sequences.
    overlays_raw = props.get("dynamicOverlays")
    if isinstance(overlays_raw, list):
        overlays_clean: list[dict] = []
        for item in overlays_raw:
            if not isinstance(item, dict):
                continue
            overlay_type = str(item.get("type", "")).strip()
            if not overlay_type:
                continue
            try:
                start_s = max(0.0, float(item.get("startSeconds", 0.0) or 0.0))
                dur_s = float(item.get("durationSeconds", 0.0) or 0.0)
            except (TypeError, ValueError):
                continue
            if dur_s <= 0.0:
                continue
            overlay = {
                "type": overlay_type,
                "startSeconds": round(start_s, 3),
                "durationSeconds": round(max(0.1, dur_s), 3),
            }
            if item.get("text"):
                overlay["text"] = str(item.get("text"))[:240]
            if item.get("style"):
                overlay["style"] = str(item.get("style"))
            if item.get("position"):
                overlay["position"] = str(item.get("position"))
