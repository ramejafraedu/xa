"""pipeline/renderer_remotion.py - Render con Remotion usando el schema enriquecido"""
...
def render_with_remotion(
    ...
    music_path: str = "",
    duration: float = 30.0,
    nicho_slug: str = "default"
) -> bool:
    """
    Renderiza el video usando Remotion + el schema con overlays automáticos.
    """
    try:
        remotion_project = Path(__file__).parent.parent / "remotion-composer"
        
        if not remotion_project.exists():
            logger.error("remotion-composer folder no existe")
            return False

        # Preparar props para Remotion
        props = {
        ...
        cmd = [
            "npx", "remotion", "render",
            str(remotion_project / "src" / "index.tsx"),
            "CinematicRenderer",   # Puedes cambiar a Explainer, TalkingHead, AnimeScene, etc.
            str(output_path),
        ...
        logger.info(f"Renderizando con Remotion: {output_path}")
        result = subprocess.run(cmd, capture_output=True, text=True, timeout=400)

        if result.returncode == 0:
            logger.success(f"✅ Video renderizado con Remotion → {output_path}")
            return True
        else:
            logger.error(f"Remotion falló: {result.stderr[-500:]}")
            return False

    except subprocess.TimeoutExpired:
        logger.error("Remotion timeout (más de 6 minutos)")
        return False
    except Exception as e:
        logger.error(f"Error en render_with_remotion: {e}")
        return False
